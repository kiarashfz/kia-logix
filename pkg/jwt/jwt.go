...
func CreateToken(secret []byte, claims *UserClaims) (string, error) {
	return jwtLib.NewWithClaims(jwtLib.SigningMethodHS512, claims).SignedString(secret)
}
...
func ParseToken(tokenString string, secret []byte) (*UserClaims, error) {
	...
	token, err := jwtLib.ParseWithClaims(tokenString, &UserClaims{}, func(t *jwtLib.Token) (interface{}, error) {
		return secret, nil
	})
